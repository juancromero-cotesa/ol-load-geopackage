...
promiseSqlWasmLoaded.catch((error) => {
...
export default function (gpkgFile, displayProjection) {
  ...
  return Promise.all([promiseSqlWasmLoaded, gpkgPromise])
    .then(([sqlWasm, gpkgArrayBuffer]) =>
      processGpkgData(gpkgFile, gpkgArrayBuffer, sqlWasm, displayProjection)
    )
    .catch((error) => {
      throw error;
    });
    ...
    oReq.onreadystatechange = function () {
      ...
      if (this.readyState == 4) {
        ...
        if (this.status === 200 && gpkgArrayBuffer) {
          ...
          fail(
            new Error("Requested GPKG file could not be loaded: " + gpkgFile)
          );
        }
      }
    };
